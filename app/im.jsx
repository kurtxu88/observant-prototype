/* ============================================================
   OBSERVANT app — 1:1 IM surface (slide-over conversation)
   ============================================================ */
...
function IMSurface({ user, onClose }) {
  ...
  const relay = () => {
    ...
    play([
      ...
      { typing: 950 },
      { msg: { t: "them", text: `One more from the ${"Northwind"} team — ${q.charAt(0).toLowerCase() + q.slice(1)}`, meta: "Observant · relaying" }, post: 1500 },
      { typing: 1200 },
  ...
  const requestLive = () => {
    ...
    play([
      ...
      { typing: 950 },
      { msg: { t: "them", text: `Happy to arrange that. ${fname} — the Northwind team would love 15 minutes to dig in. Does Thursday at 2pm work?`, meta: "Observant" }, post: 1600 },
      { typing: 1100 },
    ...
    <React.Fragment>
      ...
      <aside className="im" role="dialog" aria-label={`Conversation with ${user.name}`}>
        ...
        <footer className="im-foot">
          <span className="foot-label">Relay a question through Observant</span>
          <div className="relay-input">
            <textarea
              className="textarea" placeholder={`Ask ${fname} something — Observant threads it into the conversation…`}
