...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!partner.stripe_account_id) {
      return res.status(200).json({ ok: false, error: "partner has no connected payout account — run /api/stripe/connect first" });
    }
    if (partner.payouts_enabled === false) {
      return res.status(200).json({ ok: false, error: "payouts not enabled yet — finish Stripe onboarding" });
    }
    ...
    if (!(amount > 0)) return res.status(200).json({ ok: false, error: "no balance to pay out", amount: 0 });
    ...
    return res.status(200).json({ ok: true, amount, transferId });
  } catch (error) {
