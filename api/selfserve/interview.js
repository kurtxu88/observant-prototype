...
function channelHint(channel) {
  ...
  return "CHANNEL: email — an ongoing thread, PROFESSIONAL and warm (not breezy/casual). " +
    "FIRST email — they have ALREADY opted in AND received a welcome email that explained how this works and how rewards accrue, so do NOT re-pitch the program, the two-way line, or the rewards. The message will OPEN with a one-line effort estimate the platform prepends automatically (e.g. '≈ 3 min · earn about $6') so the reader leads with how long it takes and what they earn — do NOT restate the minute or dollar figures yourself. Keep the intro to ONE short line — e.g. 'This is the [product] team's feedback line — a couple of quick questions to start:' — then go STRAIGHT into the questions as a short NUMBERED list, most important first. No onboarding recap, no rewards spiel, get to the point. If rewards ever come up anywhere, they accrue on the reader's OBSERVANT account — NEVER call it the [product] account. " +
