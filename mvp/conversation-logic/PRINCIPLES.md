...
### P19 — Bidirectional channel, made explicit to users (anti-spam), added 2026-06-17
The relationship is two-way, and we *say so* on every user-facing surface (invite, first email, Telegram open, answer form): **reach out anytime something goes wrong or you want to share product feedback — not only when we ask — and genuine feedback you send on your own earns rewards the same way.** This is both a value-add and a core anti-spam move: a channel the user can *initiate* feels like a line they own, not one-way pestering. It pairs with P7 (capture proactive/off-guide input as signal — don't redirect) — now with the reward framing surfaced to the user. *Encoded in: channelHint (email + telegram), send-email footers (light + deep), answer form, join invite.*
